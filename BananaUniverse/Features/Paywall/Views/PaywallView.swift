    ...
    private var headerSection: some View {
        VStack(spacing: 12) {
            Image(systemName: "bolt.fill")
                ...
                .foregroundStyle(
                    LinearGradient(
                        colors: [DesignTokens.Brand.accent(.light), DesignTokens.Semantic.error],
                        startPoint: .topLeading,
                        endPoint: .bottomTrailing
                    )
                )
    ...
    private var creditsDisplaySection: some View {
        HStack(spacing: 16) {
            Image(systemName: "sparkles")
                .font(.title)
                .foregroundColor(DesignTokens.Brand.accent(.light))
    ...
    private var purchaseFlowSection: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                ...
                .padding()
                .background(DesignTokens.Brand.primary(.light).opacity(0.1))
                .cornerRadius(12)
                .overlay(
                    RoundedRectangle(cornerRadius: 12)
                        .stroke(DesignTokens.Brand.primary(.light), lineWidth: 2)
                )
            }
            .buttonStyle(.plain)
...
struct ProductCard: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .padding()
            .background(isSelected ? DesignTokens.Brand.primary(.light).opacity(0.15) : Color.white.opacity(0.8))
            .cornerRadius(16)
            .overlay(
                RoundedRectangle(cornerRadius: 16)
                    .stroke(isSelected ? DesignTokens.Brand.primary(.light) : Color.clear, lineWidth: 2)
            )
