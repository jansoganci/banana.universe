...
class SupabaseService: ObservableObject {
    ...
    func fetchUserJobs(
        ...
        print("📚 [SupabaseService] Fetching jobs for user: \(userState.identifier)")
        print("🔍 [SupabaseService] User authenticated: \(userState.isAuthenticated)")
        print("🔍 [SupabaseService] Query limit: \(limit), offset: \(offset)")
        ...
        do {
            ...
            if userState.isAuthenticated {
                ...
                print("🔍 [SupabaseService] Querying by user_id: \(userState.identifier)")
                ...
                print("🔍 [SupabaseService] Executing authenticated query...")
                ...
                print("🔍 [SupabaseService] Querying by device_id using function: \(userState.identifier)")
                
                print("🔍 [SupabaseService] Executing anonymous function query...")
            ...
            if !response.isEmpty {
                print("🔍 [SupabaseService] First job details:")
                for (index, job) in response.prefix(3).enumerated() {
                    print("  Job \(index + 1): ID=\(job.id), user_id=\(job.userId?.uuidString ?? "nil"), device_id=\(job.deviceId ?? "nil"), status=\(job.status)")
                }
            } else {
                print("🔍 [SupabaseService] No jobs found in database")
            }
            
            return response
            
        } catch {
            print("❌ [SupabaseService] Failed to fetch jobs: \(error)")
